...
def load_local_workspace_config():
    ...
    if not local_workspace_config_path.exists():
        raise LocalWorkspaceConfigError(
            "Could not find a workspace.yaml file in your pipeline directory."
        )
    ...
    with open(
        ...
        try:
            ...
            os.environ["WORKSPACE_DATABASE_HOST"] = local_workspace_config["database"][
                "host"
            ]
        ...
        for slug, connection_config in local_workspace_config["connections"].items():
            ...
            if connection_config["type"] == "dhis2":
                try:
                    os.environ[f"{stringcase.constcase(slug)}_URL"] = connection_config[
                        "url"
                    ]
                    os.environ[
